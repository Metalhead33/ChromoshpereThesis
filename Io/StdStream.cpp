...
int64_t StdStream::read( void *data, int64_t size ) {
	...
	return (int64_t)fread( data, 1, size, chandle );
}
...
int64_t StdStream::seek( int64_t position ) {
	...
	return (int64_t)fseek( chandle, position, SEEK_SET );
}
...
int64_t StdStream::write( const void *data, int64_t size ) {
	...
	return (int64_t)fwrite( data, 1, size, chandle );
}
...
bool StdStream::open( const std::string &newpath, bool ro ) {
	...
	fclose( chandle );
	if ( ro )
	chandle = fopen( newpath.c_str( ), "rb" );
	else {
	chandle = fopen( newpath.c_str( ), "r+b" );
	if ( !chandle )
		chandle = fopen( newpath.c_str( ), "wb" );
	}
	if ( chandle ) {
	path = newpath;
	active = true;
...
char StdStream::getc( ) { return fgetc( chandle ); }
