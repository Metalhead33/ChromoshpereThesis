...
// Reimplementation of C++17 std::size() and std::data()
	...
	template < typename ElementType, std::ptrdiff_t Extent > class span {
	static_assert(
	...
	template < std::ptrdiff_t Offset,
		   std::ptrdiff_t Count = dynamic_extent >
	TCB_SPAN_CONSTEXPR11 subspan_return_t< Offset, Count >
	subspan( ) const {
	...
	TCB_SPAN_CONSTEXPR14 reference at( index_type idx ) const {
		...
		std::snprintf( msgbuf, sizeof( msgbuf ),
				   "Index %td is out of range for span of size %td",
				   idx, size( ) );
	...
	TCB_SPAN_ARRAY_CONSTEXPR const_reverse_iterator crbegin( ) const
		noexcept {
		return const_reverse_iterator( cend( ) );
	}

	TCB_SPAN_ARRAY_CONSTEXPR const_reverse_iterator crend( ) const
		noexcept {
...
namespace std {
	...
	class tuple_size< TCB_SPAN_NAMESPACE_NAME::span< E, S > >
	: public integral_constant< size_t, S > {};

	template < typename E >
	class tuple_size< TCB_SPAN_NAMESPACE_NAME::span<
	E, TCB_SPAN_NAMESPACE_NAME::dynamic_extent > >; // not defined
