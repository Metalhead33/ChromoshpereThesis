...
Mh::ImageWrapper blurAndCopy( const Mh::ImageWrapper &image, int blurRadius ) {
	...
	Mh::ImageWrapper img2( Mh::ImageFormat::FLOAT, image.getWidth( ),
			   image.getHeight( ), 32 );
	GreyscalePixels output2( static_cast< float * >( img2.getBytes( ) ),
...
Mh::ImageWrapper blurAndCopy( const Mh::ImageWrapper &image,
				  BlurType blurType ) {
	if ( image.getFormat( ) != Mh::ImageFormat::FLOAT )
	throw std::runtime_error( GreyscaleOnlyError );
	Mh::ImageWrapper img( Mh::ImageFormat::FLOAT, image.getWidth( ),
